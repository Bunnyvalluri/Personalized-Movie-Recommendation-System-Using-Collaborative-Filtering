...
import { ChevronLeft, ChevronRight, Star, Plus, Play } from 'lucide-react';
        ...
        >
          {movies.map((movie: any) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="relative min-w-[220px] md:min-w-[320px] aspect-[16/9] cursor-pointer group/card"
            >
              <div className="relative h-full w-full overflow-hidden rounded-xl bg-surface-raised border border-white/5 shadow-2xl transition-all duration-300 group-hover/card:ring-2 group-hover/card:ring-[#e50914]/50 group-hover/card:shadow-[#e50914]/10">
                ...
                {/* Status Tags (Requested by User) */}
                {movie.status_tag && (
                    <div className="absolute bottom-3 left-3 flex gap-1 z-10">
                        <span className="bg-[#e50914] text-[9px] font-black px-1.5 py-0.5 rounded shadow-lg uppercase tracking-tight text-white">
                            {movie.status_tag}
                        </span>
                        {movie.status_tag === "NEW EPISODE" && (
                            <span className="bg-white text-black text-[9px] font-black px-1.5 py-0.5 rounded shadow-lg uppercase tracking-tight flex items-center gap-1">
                                <Play className="h-2 w-2 fill-current" /> Watch Now
                            </span>
                        )}
                    </div>
                )}
                ...
                <div className="absolute bottom-0 p-4 w-full transform translate-y-4 group-hover/card:translate-y-0 transition-transform duration-300 opacity-0 group-hover/card:opacity-100">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex gap-1">
                            ...
                            <span className="text-[10px] uppercase font-bold text-white/70 tracking-widest">Premium Collection</span>
                        </div>
                        <Plus className="h-4 w-4 bg-white/20 rounded-full" />
                    </div>
                    <h4 className="font-bold text-sm lg:text-base line-clamp-1 uppercase tracking-tight">{movie.title || movie.name}</h4>
                </div>
              </div>
            </motion.div>
          ))}
